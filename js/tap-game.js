class TapGame {
  constructor(toast, sound) {
    ...
    this.difficultySettings = {
      easy: { options: 2, label: 'Лёгкий' },
      normal: { options: 3, label: 'Средний' },
      hard: { options: 4, label: 'Сложный' }
    };
  ...
  getComboMultiplier() {
    ...
    const index = Math.min(this.combo, this.comboMultipliers.length) - 1;
  ...
  renderIdle() {
    ...
    this.container.innerHTML =
      ...
      '<div class="tap-stat"><div class="tap-stat-value">0</div><div class="tap-stat-label">Серия</div></div>' +
      '</div>' +
      ...
      '</div>' +
      '<p class="tap-hint">Ответ только по нажатию. Быстрый ответ (&lt;3 сек) даёт +5 бонусных очков. На ПК: P — пауза.</p>';
  ...
  renderGame() {
    ...
    this.container.innerHTML =
      ...
      '<div class="tap-stat"><div class="tap-stat-value" id="tap-combo-stat">0</div><div class="tap-stat-label">Серия</div></div>' +
      '</div>' +
      '<div class="tap-question-card" id="tap-question-card">' +
      '<div class="tap-combo" id="tap-combo-badge">x1</div>' +
      ...
      '<div class="tap-controls">' +
      '<button class="btn btn-secondary" id="btn-pause-tap"><i class="fas fa-pause"></i> Пауза</button>' +
  ...
  nextWord() {
    ...
    if (promptEl) promptEl.textContent = 'Найди перевод';
    ...
    if (optionsEl) {
      ...
      optionsEl.innerHTML = this.options
        .map(
          (option) =>
            '<button class="tap-option" type="button" data-answer="' +
  ...
  handleAnswer(selected, event) {
    ...
    if (correct) {
      ...
      const points = Math.round((10 + speedBonus) * mult);
      this.score += points;
  ...
  updateStats() {
    ...
    if (comboEl) comboEl.textContent = String(this.combo);
    if (badge) badge.textContent = 'x' + this.getComboMultiplier();
  }
